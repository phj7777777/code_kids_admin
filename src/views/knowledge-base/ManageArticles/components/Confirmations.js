...
import {apiDeleteArticle} from "../../../../services/KnowledgeBaseService";
...
const Confirmations = ({data}) => {
	...
	const onArticleDeleteConfirm = async () => {
		...
		await apiDeleteArticle(selected.id)
		const removedData = allArticles.map(categories => {
			if (categories.value === selected.categoryValue) {
				categories.articles = categories.articles.filter(article => article._id !== selected.id)
			}
			return categories
		})
		dispatch(toggleArticleDeleteConfirmation(false))
